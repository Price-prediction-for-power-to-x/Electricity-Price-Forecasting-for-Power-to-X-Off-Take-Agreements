...
For each bidding zone and each week of the following year, the year-ahead forecast provides **minimum and maximum total load values**. Thus, the dataset does not provide an hourly load profile; rather, it provides the forecasted weekly load range (minimum and maximum load) for the bidding zone. 
